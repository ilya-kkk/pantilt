...
def set_visibility(mode="normal", show_axes=False):
    ...
    for obj in shape_objects:
        ...
        if mode == "pan_focus":
            visible = (
                label.startswith("BASE_pan_")
                or label == "PAN_ROTATING_one_piece_platform_journal_120T_GT2"
            )
            if label.startswith("BASE_rubber"):
                visible = False
        elif mode == "tilt_focus":
            visible = (
                label == "PAN_ROTATING_one_piece_platform_journal_120T_GT2"
            ...
            visible = label == "PAN_ROTATING_one_piece_platform_journal_120T_GT2"
        ...
        elif mode == "pan_bearing_focus":
            visible = (
                label == "BASE_three_leg_frame_51107_608_motor_mount"
                or label.startswith("BASE_pan_51107")
                or label == "BASE_pan_608_radial_bearing_press_fit"
                or label == "PAN_ROTATING_one_piece_platform_journal_120T_GT2"
            )
        elif mode == "pan_part_focus":
            visible = label == "PAN_ROTATING_one_piece_platform_journal_120T_GT2"
        elif mode == "pan_motor_focus":
            visible = (
                label == "BASE_three_leg_frame_51107_608_motor_mount"
                or label.startswith("BASE_pan_motor_")
                or label == "BASE_pan_GT2_350mm_belt"
                or label == "PAN_ROTATING_one_piece_platform_journal_120T_GT2"
            )
        elif mode == "pan_drive_focus":
            visible = (
                label == "PAN_ROTATING_one_piece_platform_journal_120T_GT2"
                or label == "BASE_pan_GT2_350mm_belt"
                or label == "BASE_pan_motor_20T_pulley"
                or label == "BASE_pan_motor_20T_flanges"
            )
...
def save_view(name, view_command, mode="normal", show_axes=False):
    ...
    apply_style()
    if mode in ("saddle_focus", "service_focus", "pan_bearing_focus"):
        for obj in shape_objects:
            ...
            elif mode == "pan_bearing_focus" and obj.Label == "PAN_ROTATING_one_piece_platform_journal_120T_GT2":
...
def make_reference_comparison():
    ...
    command = [
        ...
        "-y",
        "-i",
        os.path.join(SCREEN_DIR, "01_isometric_v8.png"),
        "-i",
        os.path.join(SCREEN_DIR, "07_tilt_drive_focus_v8.png"),
        "-i",
        os.path.join(SCREEN_DIR, "02_front_v8.png"),
    ]
    ...
    command.extend(
        [
            ...
            "-update",
            "1",
            os.path.join(SCREEN_DIR, "11_reference_comparison_v8.png"),
        ]
    )
...
def make_exploded():
    ...
    for obj in shape_objects:
        ...
        if label == "PAN_ROTATING_one_piece_platform_journal_120T_GT2":
            delta = App.Vector(0, 0, 18)
        elif label == "BASE_pan_51107_upper_shaft_washer":
            delta = App.Vector(0, 0, 34)
        elif label == "BASE_pan_51107_ball_cage":
            delta = App.Vector(0, 0, 24)
        elif label == "BASE_pan_51107_lower_housing_washer":
            delta = App.Vector(0, 0, 14)
        elif label == "BASE_pan_608_radial_bearing_press_fit":
            delta = App.Vector(0, 0, -10)
...
def make_pan_bearing_section():
    restore_shapes()
    set_visibility("pan_bearing_focus", show_axes=False)
    section_box = Part.makeBox(220, 110, 125, App.Vector(-140, 0, 0))
    for obj in shape_objects:
        if obj.ViewObject.Visibility:
            obj.Shape = obj.Shape.common(section_box)
    doc.recompute()


def make_pan_drive_closeup():
    restore_shapes()
    set_visibility("pan_drive_focus", show_axes=False)
    target = next(
        obj for obj in shape_objects
        if obj.Label == "PAN_ROTATING_one_piece_platform_journal_120T_GT2"
    )
    target.Shape = target.Shape.common(
        Part.makeBox(300, 220, 61, App.Vector(-150, -110, 0))
    )
    doc.recompute()
...
save_view("01_isometric_v8.png", view.viewIsometric)
print("SCREENSHOT_STAGE", "saved_01")

set_pose()
save_view("02_front_v8.png", view.viewFront)
print("SCREENSHOT_STAGE", "saved_02")
save_view("03_side_v8.png", view.viewRight)
print("SCREENSHOT_STAGE", "saved_03")
save_view("04_top_v8.png", view.viewTop)
print("SCREENSHOT_STAGE", "saved_04")

make_exploded()
save_view("05_exploded_v8.png", view.viewIsometric)
print("SCREENSHOT_STAGE", "saved_05")

make_pan_drive_closeup()
save_view("06_pan_drive_focus_v8.png", view.viewTop, mode="pan_drive_focus")
print("SCREENSHOT_STAGE", "saved_06")
save_view("07_tilt_drive_focus_v8.png", view.viewIsometric, mode="tilt_focus", show_axes=True)
print("SCREENSHOT_STAGE", "saved_07")

set_pose(pan_angle=-18, tilt_angle=60)
save_view("08_tilt_plus60_v8.png", view.viewIsometric, show_axes=True)

set_pose(pan_angle=-18, tilt_angle=-60)
save_view("09_tilt_minus60_v8.png", view.viewIsometric, show_axes=True)

set_pose(pan_angle=45, tilt_angle=18)
save_view("10_pan45_tilt18_v8.png", view.viewIsometric)
...
time.sleep(0.2)
view.saveImage(
    os.path.join(SCREEN_DIR, "12_gt2_2mr_profile_closeup_v8.png"),
    1600,
    1200,
    "White",
)
...
set_pose()
save_view("13_direct_tilt_motor_mount_v8.png", view.viewIsometric, mode="direct_motor_focus")
print("SCREENSHOT_STAGE", "saved_13_direct_motor")
save_view("14_integrated_nema17_tension_slots_v8.png", view.viewFront, mode="motor_slots_focus")
print("SCREENSHOT_STAGE", "saved_14_motor_slots")

set_pose()
save_view("15_short_camera_saddle_v8.png", view.viewTop, mode="saddle_focus", show_axes=True)
print("SCREENSHOT_STAGE", "saved_15_short_saddle")
save_view("16_open_lcd_hdmi_clearance_v8.png", view.viewRight, mode="service_focus")
print("SCREENSHOT_STAGE", "saved_16_service_clearance")

make_pan_bearing_section()
save_view("17_pan_51107_608_section_v8.png", view.viewFront, mode="pan_bearing_focus")
print("SCREENSHOT_STAGE", "saved_17_pan_bearing_section")

set_pose()
save_view("18_integral_pan_120T_bottom_v8.png", view.viewBottom, mode="pan_part_focus")
print("SCREENSHOT_STAGE", "saved_18_integral_pan_bottom")
save_view("19_pan_motor_shaft_down_v8.png", view.viewIsometric, mode="pan_motor_focus")
print("SCREENSHOT_STAGE", "saved_19_pan_motor_down")
