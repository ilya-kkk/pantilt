...
DOC_NAME = "pantilt_hc_v760_reference_v2"
...
def build_pan_rotating(doc, pan_group, objects):
    ...
    for side in (-1, 1):
        gusset = prism_y([(-70, 83), (-20, 83), (-20, 112)], 8, side * 47 - 4)
    ...
    motor_slide = box(70, 3, 70, (P["tilt_motor_x"], -75, P["tilt_motor_z"]))
    motor_slide = motor_slide.cut(
        slot_y(
            35,
            P["nema_pilot_d"] + 1,
            5,
            (P["tilt_motor_x"], -75, P["tilt_motor_z"]),
            along="z",
        )
    )
    for sx in (-1, 1):
    ...
    standoffs = Part.makeCompound(
        [
            cyl_y(4.5, 17.5, (P["tilt_motor_x"] + sx * 28, -64.75, P["tilt_motor_z"] + sz * 28))
            for sx in (-1, 1)
            for sz in (-1, 1)
        ]
    )
    ...
    objects["PAN_ROTATING_tilt_motor_NEMA17"] = add(
        ...
        box(P["nema_w"], P["nema_l"], P["nema_w"], (P["tilt_motor_x"], -100.5, P["tilt_motor_z"])),
        "motor",
    )
    objects["PAN_ROTATING_tilt_motor_pilot"] = add(
        ...
        cyl_y(P["nema_pilot_d"] / 2, 2, (P["tilt_motor_x"], -75.5, P["tilt_motor_z"])),
        "steel",
    )
    objects["PAN_ROTATING_tilt_motor_shaft"] = add(
        ...
        cyl_y(2.5, 17, (P["tilt_motor_x"], -68, P["tilt_motor_z"])),
        "steel",
    )
    ...
    driven_pulley, driven_flanges, driven_teeth = pulley_y(
        P["tilt_driven_pitch_d"],
        P["shaft_d"],
        P["belt_w"],
        (P["tilt_axis_x"], P["tilt_belt_y"], P["tilt_axis_z"]),
        teeth=P["tilt_driven_teeth"],
    )
    objects["PAN_ROTATING_tilt_80T_pulley"] = add(
        doc, pan_group, "PAN_ROTATING_tilt_80T_pulley", driven_pulley, "pulley"
    )
    objects["PAN_ROTATING_tilt_80T_flanges"] = add(
        doc, pan_group, "PAN_ROTATING_tilt_80T_flanges", driven_flanges, "pulley"
    )
    objects["PAN_ROTATING_tilt_80T_exact_tooth_count"] = add(
        doc, pan_group, "PAN_ROTATING_tilt_80T_exact_tooth_count", driven_teeth, "pulley"
    )
...
def build_tilt_rotating(doc, tilt_group, objects):
    objects["TILT_ROTATING_8mm_steel_axle"] = add(
        doc,
        tilt_group,
        "TILT_ROTATING_8mm_steel_axle",
        cyl_y(P["shaft_d"] / 2, 144, (P["tilt_axis_x"], 0, P["tilt_axis_z"])),
        "steel",
    )
    objects["TILT_ROTATING_left_axle_collar"] = add(
        doc,
        tilt_group,
        "TILT_ROTATING_left_axle_collar",
        ring_y(16, P["shaft_d"], 3, (0, 63, P["tilt_axis_z"])),
        "steel",
    )
    ...
    for x in (-48, 48):
        ...
        plate = plate.cut(slot_z(22, 5, 7, (x, -28, plate_z), along="x"))
    objects["TILT_ROTATING_camera_plate"] = add(
        doc, tilt_group, "TILT_ROTATING_camera_plate", plate, "printed_dark"
    )

    for side in (-1, 1):
        objects[f"TILT_ROTATING_cradle_cheek_{side}"] = add(
            doc,
            tilt_group,
            f"TILT_ROTATING_cradle_cheek_{side}",
            box(132, 6, 44, (0, side * 37, 161)),
            "printed",
        )
        objects[f"TILT_ROTATING_axle_hub_{side}"] = add(
            doc,
            tilt_group,
            f"TILT_ROTATING_axle_hub_{side}",
            ring_y(26, P["shaft_d"], 8, (0, side * 37, P["tilt_axis_z"])),
            "printed_dark",
        )
...
def validate_geometry(doc, objects):
    pair_names = [
        ...
        ("tilt_right_yoke_vs_driven_pulley", "PAN_ROTATING_right_yoke_motor_mount", "PAN_ROTATING_tilt_80T_pulley"),
        ...
        ("camera_plate_vs_tilt_motor_nominal", "TILT_ROTATING_camera_plate", "PAN_ROTATING_tilt_motor_NEMA17"),
    ]
    pair_results = {}
    ...
    moving_labels = [
        "TILT_ROTATING_camera_plate",
        "TILT_ROTATING_cradle_cheek_-1",
        "TILT_ROTATING_cradle_cheek_1",
        "CAMERA_HC_V760_body_placeholder",
        "CAMERA_HC_V760_lens",
        "CAMERA_HC_V760_rear_battery",
    ]
    ...
    result = {
        ...
        "passed": len(invalid) == 0 and all(value <= 0.01 for value in all_volumes),
    }
...
def build():
    ...
    assembly.Label = "PAN_TILT_REFERENCE_V2"
